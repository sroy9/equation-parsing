...
//		System.out.println("Output from inference : "+beam2.element().getFirst());
	...
	public Pair<LcaY, Double> getBottomUpBestParse(
		...
		return new Pair<LcaY, Double>(y, finalScore);
	}
	...
	public double getScore(Node node, WeightVector wv, LcaX x) {
		return wv.dotProduct(featGen.getExpressionFeatureVector(x, node));
	}
	
	public LcaY getLatentBestStructure(
			LcaX x, LcaY gold, WeightVector wv) {
		LcaY best = null;
		double bestScore = -Double.MAX_VALUE;
		if(gold.varTokens.keySet().size() == 1) {
			for(Integer tokenIndex : gold.varTokens.get("V1")) {
				LcaY yNew = new LcaY(gold);
