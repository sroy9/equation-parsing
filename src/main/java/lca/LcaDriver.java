...
//		model.wv = learner.train(train);
		model.wv = latentSVMLearner(learner, train, 
				(LcaInfSolver) model.infSolver, 2);
		lm.setAllowNewFeatures(false);
		model.saveModel(modelPath);
	}

	public static WeightVector latentSVMLearner(
			Learner learner, SLProblem sp, LcaInfSolver infSolver, 
		...
		for(int i=0; i<maxIter; ++i) {
			...
			for(int j=0; j<sp.goldStructureList.size(); ++j) {
				LcaX prob = (LcaX) sp.instanceList.get(j);
				LcaY gold = (LcaY) sp.goldStructureList.get(j);
//				System.out.println("GetLatent : "+prob.problemIndex+" : "+gold);
				LcaY bestLatent = infSolver.getLatentBestStructure(prob, gold, wv);
