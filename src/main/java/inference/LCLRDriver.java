...
public class LCLRDriver {
	
	public static boolean useSPforNumoccur = true;
	...
	public static double testModel(SLModel numOccurModel, SLModel lastTwoModel, 
		...
		for (int i = 0; i < sp.instanceList.size(); i++) {
			...
			TreeY pred = LCLRInfSolver.getBestStructure(prob, numOccurModel, lastTwoModel, gold);
	...
	public static void main(String args[]) throws Exception {
		if(args[0].equals("SP")) {
			LCLRDriver.useSPforNumoccur = true; 
		} else {
			LCLRDriver.useSPforNumoccur = false;
		}
		LCLRDriver.crossVal();
