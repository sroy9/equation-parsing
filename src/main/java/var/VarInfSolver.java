...
//		System.out.println("Inference called with "+prob.problemIndex);
		...
		PairComparator<VarY> pairComparator = 
				new PairComparator<VarY>() {};
		MinMaxPriorityQueue<Pair<VarY, Double>> beam1 = 
				MinMaxPriorityQueue.orderedBy(pairComparator)
				.maximumSize(50).create();
		MinMaxPriorityQueue<Pair<VarY, Double>> beam2 = 
				MinMaxPriorityQueue.orderedBy(pairComparator)
				.maximumSize(50).create();
...
//		System.out.println("Beam size : "+beam1.size());
		...
		// Equation generation
		for(Pair<VarY, Double> pair : beam1) {
...
//		System.out.println("Output from inference : "+beam2.element().getFirst());
	...
	public Pair<VarY, Double> getBottomUpBestParse(
			VarX x, Pair<VarY, Double> pair, WeightVector wv) {
		VarY y = pair.getFirst();
		...
		return new Pair<VarY, Double>(y, finalScore);
	}
	
	public List<Pair<List<Node>, Double>> enumerateSingleMerge(
			Pair<List<Node>, Double> state, WeightVector wv, VarX x) {
	...
	public List<Pair<Node, Double>> enumerateMerge(
			Node node1, Node node2, WeightVector wv, VarX x) {
	...
	public double getScore(Node node, WeightVector wv, VarX x) {
