...
public class CompDriver {
	...
	public static SLProblem getSP(List<SimulProb> simulProbList) throws Exception {
		...
		for (SimulProb simulProb : simulProbList) {
			for(int i=0; i<simulProb.equations.size(); ++i) {
				CompX semX = new CompX(simulProb, "R"+(i+1));
				CompY semY = new CompY(simulProb.equations.get(i));
	...
	private static double testModel(String modelPath, SLProblem sp)
		...
		for (int i = 0; i < sp.instanceList.size(); i++) {
			CompX prob = (CompX) sp.instanceList.get(i);
			CompY gold = (CompY) sp.goldStructureList.get(i);
			CompY pred = (CompY) model.infSolver.getBestStructure(
			...
			for(Pair<CompY, Double> pair : ((CompInfSolver) model.infSolver).beam) {
				if (CompY.getLoss(gold, pair.getFirst()) < 0.00001) {
					beamAcc += 1.0;
					break;
				}
			}
			if (CompY.getLoss(gold, pred) < 0.00001) {
				acc += 1.0;
			} else {
				...
				System.out.println("Loss : "+CompY.getLoss(gold, pred));
			}
		}
	...
	public static void trainModel(String modelPath, SLProblem train)
		...
		model.infSolver = new CompInfSolver(fg, CompInfSolver.extractTemplates(train));
